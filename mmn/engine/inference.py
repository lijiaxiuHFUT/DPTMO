...
def compute_on_dataset(model, data_loader, device, timer=None):
    ...
    for batch in data_loader:  # use tqdm(data_loader) for showing progress bar
        ...
        with torch.no_grad():
            if timer:
                timer.tic()
            _,_,contrastive_output, iou_output, iou_fusion_output= model(batches.to(device))
            if timer:
                if not device.type == 'cpu':
                    torch.cuda.synchronize()
                timer.toc()
            contrastive_output, iou_output, iou_fusion_output = [o.to(cpu_device) for o in contrastive_output], [o.to(cpu_device) for o in iou_output], [o.to(cpu_device) for o in iou_fusion_output]
        results_dict.update(
            {video_id: {'contrastive': result1, 'iou': result2, 'iou_fusion': result3} for video_id, result1, result2, result3 in zip(idxs, contrastive_output, iou_output, iou_fusion_output)}
        )
    return results_dict
