...
def evaluate(cfg, dataset, predictions, nms_thresh, recall_metrics=(1, 5)):
    ...
    for idx, result2d in tqdm(enumerate(predictions)):   # each video
        score2d = torch.pow(result2d['contrastive'] * 0.5 + 0.5, cfg.TEST.CONTRASTIVE_SCORE_POW) * result2d['iou'] * result2d['iou_fusion']
        ...
        for gt_moment, pred_score2d in zip(gt_moments, score2d):  # each sentence
            ...
            for i, r in enumerate(recall_metrics):
                if(r == 1):
                    ...
                    start_s, end_e = float(moments[:r][0][0]), float(moments[:r][0][1])
                    result[str(tot)] = [strftime("%H:%M:%S", gmtime(start_s)), strftime("%H:%M:%S", gmtime(end_e))]
                    tot = tot + 1
                import pdb;pdb.set_trace()
